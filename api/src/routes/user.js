...
router.get('/login', async (req, res) => {
  ...
  try {
    if (users.length > 0) {
      ...
      if (user.length > 1) return res.status(200).json({ message: "Error! Hay más de un usuario con ese mail y contraseña", login: false }) // Esto nunca debería ocurrir, aún así lo agrego
      var resp = {
