...
function Envido() {
    ...
    const intro = 'Cada jugador puede cantar envido en su primer turno durante la mano, siempre que sea antes de la propuesta del truco ya que una vez abierto el envido el truco queda aplazado hasta resolver el envido_ El envido es una apuesta en la que se pone en juego cuál es el jugador que tiene una puntuación más elevada según las siguientes reglas:  - Se debe tener dos cartas del mismo palo, se suman sus numeros y al resultado se le suman 20 puntos más. Si no tiene dos cartas del mismo palo se tendrá en cuenta el valor de la carta mayor (en índice numeral)_ Si tuviera tres cartas del mismo palo solo se sumarán los valores de las dos cartas más altas y le deben sumar 20 puntas más. A estos efectos, el valor de las figuras es cero.'
    const cards = [
        {   id: 1,
            img : '/cards/39.webp',
            txt: 'Tres de Oro'},
    ...
    return (
        <div className={styles.board}>
            ...
            {
                 state.cards ? state.cards.map(c => <img className={styles.card} src={process.env.PUBLIC_URL + c.img} alt={c.txt} >
                    </img>
                   ) : null
            } </div>
             <div >
